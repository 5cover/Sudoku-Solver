...
int grid_load(FILE *inStream, tGrid *g) {
  ...
  // grid to store them.
  uint32_t *gridValues =
      check_alloc(array2d_malloc(gridValues, g->SIZE, g->SIZE), "gridValues");

  if (fread(gridValues, sizeof *gridValues, g->SIZE * g->SIZE, inStream) !=
      g->SIZE * g->SIZE) {
    free(gridValues);
  ...
  g->_isColumnFree =
      check_alloc(array2d_malloc(g->_isColumnFree, g->SIZE, g->SIZE + 1),
                  "grid _isColumnFree array");
  g->_isRowFree =
      check_alloc(array2d_malloc(g->_isRowFree, g->SIZE, g->SIZE + 1),
                  "grid _isRowFree array");
  g->_isBlockFree =
      check_alloc(array3d_malloc(g->_isBlockFree, g->N, g->N, g->SIZE + 1),
  ...
  for (tIntSize r = 0; r < g->SIZE; r++) {
    for (tIntSize c = 0; c < g->SIZE; c++) {
      ...
      cell->hasCandidate =
          check_alloc(array_calloc(cell->hasCandidate, g->SIZE + 1),
