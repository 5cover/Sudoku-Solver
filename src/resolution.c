...
bool technique_hiddenSingleton(tGrid *grid, tIntSize row, tIntSize column) {
    ...
    // Row
    if ((candidate = technique_hiddenSingleton_findUniqueCandidate(
             grid, row, row + 1, 0, grid->SIZE, &candidatePos))
        != 0) {
        grid_cell_provideValue(grid, candidatePos.row, candidatePos.column,
            candidate);
    ...
    // Column
    if ((candidate = technique_hiddenSingleton_findUniqueCandidate(
             grid, 0, grid->SIZE, column, column + 1, &candidatePos))
        != 0) {
        grid_cell_provideValue(grid, candidatePos.row, candidatePos.column,
            candidate);
        grid_removeCandidateFromBlock(grid, candidatePos.row, candidatePos.column,
...
bool technique_hiddenPair(tGrid *grid, tIntSize row, tIntSize column) {
    ...
    // Row
    progress |= (cell_candidate_count(*cellRowColumn) >= 2 && technique_hiddenPair_findPair(grid, row, row + 1, 0, grid->SIZE, pairCellPositions, candidates)) && technique_hiddenPair_removePairCells(grid, pairCellPositions, candidates);
    // Column
    progress |= (cell_candidate_count(*cellRowColumn) >= 2 && technique_hiddenPair_findPair(grid, 0, grid->SIZE, column, column + 1, pairCellPositions, candidates)) && technique_hiddenPair_removePairCells(grid, pairCellPositions, candidates);
