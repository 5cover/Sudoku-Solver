...
bool technique_hiddenSingleton(tGrid *grid, tIntSize row, tIntSize column) {
  ...
  // Row
  if ((candidate = technique_hiddenSingleton_findUniqueCandidate(
           grid, row, row + 1, 0, grid->SIZE, &candidatePos)) != 0) {
    grid_cell_provideValue(grid, candidatePos.row, candidatePos.column,
                           candidate);
  ...
  // Column
  if ((candidate = technique_hiddenSingleton_findUniqueCandidate(
           grid, 0, grid->SIZE, column, column + 1, &candidatePos)) != 0) {
    grid_cell_provideValue(grid, candidatePos.row, candidatePos.column,
                           candidate);
    grid_removeCandidateFromBlock(grid, candidatePos.row, candidatePos.column,
                                  candidate);
...
int technique_hiddenSingleton_findUniqueCandidate(
  ...
  tIntSize *candidateCounts = check_alloc(
      array_calloc(candidateCounts, grid->SIZE + 1), "candidate counts array");
  ...
  if (cell_candidate_count(*cellRowColumn) == 2) {
    ...
    for (tIntSize r = blockRow; r < blockRow + grid->N && pair.count < 2; r++) {
      for (tIntSize c = blockCol; c < blockCol + grid->N && pair.count < 2;
        ...
        pair.count +=
            (r != row || c != column) &&
            technique_nakedPair_isPairCell(grid_cellAt(*grid, r, c), pair);
      }
    }
    ...
    if (pair.count == 2) {
      ...
      for (tIntSize r = blockRow; r < blockRow + grid->N; r++) {
        for (tIntSize c = blockCol; c < blockCol + grid->N; c++) {
          bool isNotPairCell =
              !technique_nakedPair_isPairCell(grid_cellAt(*grid, r, c), pair);
          progress |= isNotPairCell &&
                      grid_cell_removeCandidate(grid, r, c, pair.candidates[0]);
          progress |= isNotPairCell &&
                      grid_cell_removeCandidate(grid, r, c, pair.candidates[1]);
        }
      }
    }
  }
...
bool technique_hiddenPair(tGrid *grid, tIntSize row, tIntSize column) {
  ...
  // Block
  progress |=
      (cell_candidate_count(*cellRowColumn) >= 2 &&
       technique_hiddenPair_findPair(grid, blockRow, blockRow + grid->N,
  ...
  progress |=
      (cell_candidate_count(*cellRowColumn) >= 2 &&
       technique_hiddenPair_findPair(grid, row, row + 1, 0, grid->SIZE,
                                     pairCellPositions, candidates)) &&
      technique_hiddenPair_removePairCells(grid, pairCellPositions, candidates);
  // Column
  progress |=
      (cell_candidate_count(*cellRowColumn) >= 2 &&
       technique_hiddenPair_findPair(grid, 0, grid->SIZE, column, column + 1,
                                     pairCellPositions, candidates)) &&
      technique_hiddenPair_removePairCells(grid, pairCellPositions, candidates);
...
bool technique_hiddenPair_findPair(tGrid const *grid, tIntSize rStart,
  ...
  for (candidates[0] = 1; candidates[0] <= grid->SIZE; candidates[0]++) {
    ...
    for (candidates[1] = candidates[0] + 1; candidates[1] <= grid->SIZE;
         candidates[1]++) {
...
bool technique_hiddenPair_findPairCells(
    ...
    tPosition pairCellPositions[PAIR_SIZE]) {
  tIntSize nbPairCells =
      1; // Count of cells found containing the pair (candidate1, candidate2).
  ...
  for (tIntSize r = rStart; r < rEnd && nbPairCells <= PAIR_SIZE + 1; r++) {
    for (tIntSize c = cStart; c < cEnd && nbPairCells <= PAIR_SIZE + 1; c++) {
      ...
      if (cell_hasCandidate(cell, candidates[0]) &&
        ...
        nbPairCellsContainingOtherCandidates +=
            cell_candidate_count(cell) > PAIR_SIZE;
    ...
    for (tIntSize colBD = colAC + 1; colBD < grid->SIZE; colBD++) {
      for (tIntSize candidate = 1; candidate <= grid->SIZE; candidate++) {
        ...
        for (tIntSize row = 0; row < grid->SIZE; row++) {
          bool colACHasCandidate =
              cell_hasCandidate(grid_cellAt(*grid, row, colAC), candidate);
          bool colBDHasCandidate =
              cell_hasCandidate(grid_cellAt(*grid, row, colBD), candidate);
        ...
        if (candidateInBothCount == 2 && candidateCounts[0] == 2 &&
            candidateCounts[1] == 2) {
          // eliminate the candidate from the rows (AB) and (CD).
          for (tIntSize c = 0; c < grid->SIZE; c++) {
            // do not remove the candidate from the cells that form the X.
            progress |= c != colAC && c != colBD &&
                        grid_cell_removeCandidate(grid, rows[0], c, candidate);
            progress |= c != colAC && c != colBD &&
                        grid_cell_removeCandidate(grid, rows[1], c, candidate);
          }
        }
      }
    }
    ...
    for (tIntSize rowCD = rowAB + 1; rowCD < grid->SIZE; rowCD++) {
      for (tIntSize candidate = 1; candidate <= grid->SIZE; candidate++) {
        ...
        for (tIntSize col = 0; col < grid->SIZE; col++) {
          bool rowABHasCandidate =
              cell_hasCandidate(grid_cellAt(*grid, rowAB, col), candidate);
          bool rowCDHasCandidate =
              cell_hasCandidate(grid_cellAt(*grid, rowCD, col), candidate);
        ...
        if (candidateInBothCount == 2 && candidateCounts[0] == 2 &&
          ...
          for (tIntSize r = 0; r < grid->SIZE; r++) {
            ...
            progress |=
                r != rowAB && r != rowCD &&
                grid_cell_removeCandidate(grid, r, columns[0], candidate);
            progress |=
                r != rowAB && r != rowCD &&
                grid_cell_removeCandidate(grid, r, columns[1], candidate);
          }
        }
      }
    }
